...
let source =
    HtmlDocument.Load("https://www.spglobal.com/platts/en/our-methodology/holiday")
...
let isPLT o (e: HtmlNode) =
    let d =
        (e.Descendants [ "div" ] |> Seq.head).InnerText() |> parseDateExact "ddMMMyyyy"
    // let desc = ( e.Descendants ["div"] |> Seq.skip 1 |> Seq.head ).InnerText()
    let offices =
        (e.Descendants [ "div" ] |> Seq.skip 2 |> Seq.head).Descendants [ "li" ]
    ...
    if (offices.Contains o || offices.Contains "All Platts offices") then
        d
    else
        None
...
let isExchange (s: string) (e: HtmlNode) =
    ...
    if (e.Descendants [ "div" ] |> Seq.skip 3 |> Seq.head).InnerText().Contains s then
        d
    else
        None

let getHol f = holRows |> Seq.choose f |> set
...
let ukHol = ukFull |> Seq.map (fun y -> y.Date) |> set
...
let pltsgpHol =
    let sgHol yr =
        let source2 =
            HtmlDocument.Load(
                "https://www.timeanddate.com/calendar/print.html?year="
                + yr
                + "&country=63&hol=9&df=1"
            ) //63 is singapore
