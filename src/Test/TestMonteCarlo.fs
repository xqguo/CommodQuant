...
type SmallInt =
    static member Int() =
        Arb.Default.Int32() |> Arb.filter (fun t -> (t > 1) && (t <= 10))
...
let ``test multivariateNormal satisfy our aimed vols and correlation`` size seed1 seed2 =
    ...
    |@ sprintf "mse of mean errors is large %f" mse_mean_errors
    .&. (mse_std_errors < 0.02)
    |@ sprintf "mse of std errors is large %f" mse_std_errors
    .&. (mse_skw_errors < 0.1)
    |@ sprintf "mse of skw errors is large %f" mse_skw_errors
    .&. (mse_krt_errors < 0.2)
    |@ sprintf "mse of krt errors is large %f" mse_krt_errors
    .&. (jbTest < criticalValue)
    |@ sprintf "Jarque Bera statistics is large: %f" jbTest
...
let ``test spread option Choi vs MM vs MC``
    ...
    .&. //choi and mc is close
    nearstr choi c' (std * 3.0 + 0.05) "Choi vs mm"
    .&. //choi and mm can be 5c diff or more
