...
let map (f : ExcelApi.Range -> 'T) (range : ExcelApi.Range) =
    range
    |> toSeq
    |> Seq.map f
...
let tryExcel () = 
    try 
        ...
        Ok excelApp 
    with |_ -> Error "Cannot get existing excel instance"
...
let tryBook filename (excel:ExcelApi.Application) = 
    ...
    with 
    |_  -> Error ("Cannot find workbook  " + filename)
...
let tryOpenBook workbookDir filename (excel:ExcelApi.Application) = 
    ...
    try 
        ...
        else Error ("Cannot find workbook  " + f)
    with 
    |_  ->         
        Error ("Cannot open workbook  " + f)
/// Get a reference to the workbook:
let trySheet sheet (book:ExcelApi.Workbook) = 
    try Ok (book.Sheets.[sheet] :?> ExcelApi.Worksheet ) with |_ -> Error ("Cannot find sheet " + sheet)

/// Get a reference to a named range:
let tryRange rangeName (sheet:ExcelApi.Worksheet) = try Ok( sheet.Range(rangeName)) with |_ -> Error ("Cannot find range " + rangeName)
...
let tryStartExcel () = 
    try 
        ...
        Ok excel
    with |_ -> Error "Cannot start excel instance"
